...
CREATE TABLE Erreur (
	id_erreur	TINYINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, 
...
INSERT INTO
	`departements` (`id_dep`, `Name`, `dep_actif`, `dep_taux`)
VALUES
	(1,	'01 - Ain', 1, 1.00),
	(2,	'02 - Aisne', 1, 1.00),
	(3, 	'03 - Allier', 1, 1.00),
...
ALTER TABLE
	candidats
ADD CONSTRAINT
	FK_departementUser_idDept
FOREIGN KEY
	(departement_user) REFERENCES departements(id_dep);
...
INSERT INTO
	candidats
VALUES
	(id_user, "Haddock", "Archibald", "ahaddock@test.fr", "admin1234", 68, 50, 0);
...
DELIMITER |
	CREATE TRIGGER
		before_delete_candidat BEFORE DELETE
	ON
		candidats FOR EACH ROW
	BEGIN
		IF OLD.archive_user <> 1
			THEN
				INSERT INTO Erreur (erreur) VALUES
					('Erreur : Le candidat il n\'est pas archivé.');
		END IF;
	END|
DELIMITER ;
